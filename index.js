...
function filterStudentsByGrade(students, grade) {
  ...
  let filteredStudents = students.filter((elem) => elem.grade === grade);
...
function logArrayElements(arr) {
  ...
  for (const item of arr) {
    console.log(`Елемент ` + arr.indexOf(item) + ` : ` + item);
  }
...
function warnAboutMaxArrayLength(arr, maxLength) {
  if (typeof maxLength !== "number" || !Array.isArray(arr)) {
    console.error(
  ...
  } else console.warn("Довжина масиву не перевищує максимально допустиму.");
...
function compareArrays(arr1, arr2) {
  arr1.sort((a, b) => a - b);
  arr2.sort((a, b) => a - b);
  const arr1const = JSON.stringify(arr1);
  const arr2const = JSON.stringify(arr2);
  console.assert(arr1const === arr2const, `Масиви не однакові!!!`);
...
function validateUserInput(userInput) {
  ...
  console.assert(userInput.name, "Помилка: ім'я користувача відсутнє!");

  if (!userInput.password) {
    console.warn("Помилка: пароль відсутній!1");
  }
  console.assert(userInput.password, "Помилка: пароль відсутній!2");
...
validateUserInput({ name: "alex", password: "pass" });
// Виведе в консоль:
// Попередження: пароль має бути довшим за 8 символів!
...
function calculateTotalPrice(products) {
  ...
  let total = products.reduce((total, elem) => total + elem.price, 0);
